...
def _deduplicate_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Remove consecutive duplicate messages to save context tokens.

    This handles simple text equality. More advanced semantic deduplication
    would require embeddings and clustering, which is out of scope for this pass.
    """
    if not messages:
        return messages
    ...
    for msg in messages:
        current_content_hash = hashlib.md5(json.dumps(msg.get("content", ""), sort_keys=True, ensure_ascii=False).encode("utf-8")).hexdigest()
        if current_content_hash != last_content_hash or msg.get("role") != "user":
            deduplicated_messages.append(msg)
            last_content_hash = current_content_hash
        else:
            # If it's a user message and content is identical to the last one, skip.
            logger.debug("Deduplicated consecutive message (hash: %s)", current_content_hash)
...
def _summarize_tool_result(tool_name: str, tool_args: str, tool_content: str) -> str:
    ...
    # Use LLM for smarter summarization if the content is long
    if content_len > 1000:  # Threshold for triggering LLM summarization
        try:
            llm_summary = call_llm(
                task = "compression",
                # Using a cheaper, faster model for summarization
                model = "minimax-cn/abab6.5-chat",
                temperature = 0.3,
                max_tokens = 150,
                stream = False,
                messages = [
                    {"role": "user", "content": f"请总结以下工具输出，提炼核心信息和关键结果，控制在100字以内。\n工具名称: {tool_name}\n工具参数: {tool_args}\n工具输出:\n{content[:2000]}"}  # Cap content to avoid token bloat
                ],
            )
            if llm_summary and llm_summary.strip():
                return f"[{tool_name}] 摘要: {llm_summary.strip()}"
        except Exception as e:
            logger.warning("LLM summarization failed for tool %s: %s", tool_name, e)
...
The user has requested that this compaction PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None) -> List[Dict[str, Any]]:
        ...
        # Phase 0: Deduplicate consecutive identical user messages
