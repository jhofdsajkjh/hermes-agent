...
# ============================================================================
# SearchSkill SRA 注入 — Select-Read-Act 三段范式 + RF 随机森林判题
# ============================================================================
_SS_PIPELINE = None

def _get_ss_pipeline():
    """懒加载 SearchSkill Pipeline，单例模式"""
    global _SS_PIPELINE
    if _SS_PIPELINE is None:
        try:
            import sys
            ss_path = r'C:\Users\Administrator\AppData\Local\hermes\search_skill_core'
            if ss_path not in sys.path:
                sys.path.insert(0, ss_path)
            from skill_pipeline import SearchSkillPipeline
            _SS_PIPELINE = SearchSkillPipeline()
            logging.getLogger(__name__).info("[SearchSkill] Pipeline 初始化完成，技能数: %d",
                                            len(_SS_PIPELINE.skill_bank.genes))
        except Exception as exc:
            logging.getLogger(__name__).warning("[SearchSkill] Pipeline 加载失败: %s", exc)
            _SS_PIPELINE = None
    return _SS_PIPELINE

def _ss_sra_preprocess(query: str, context: str = "") -> dict:
    """SRA Select 阶段 — 用 Gini 选择器选最优技能"""
    pipeline = _get_ss_pipeline()
    if pipeline is None:
        return {"query": query, "sra_instructions": [], "difficulty": "unknown"}
    try:
        result = pipeline.execute(query, context=context)
        instructions = result.get("retrieval_instructions", [])
        return {
            "query": query,
            "sra_instructions": instructions,
            "difficulty": result.get("difficulty", "unknown"),
            "selected_skills": result.get("selected_skills", []),
        }
    except Exception as exc:
        logging.getLogger(__name__).debug("[SearchSkill] SRA pre-process 失败: %s", exc)
    return {"query": query, "sra_instructions": [], "difficulty": "unknown"}

def _ss_rf_judge(query: str, results: str, sra_ctx: dict) -> dict:
    """SRA Judge 阶段 — 用随机森林验证检索质量"""
    pipeline = _get_ss_pipeline()
    if pipeline is None:
        return {"quality_pass": True, "rf_confidence": 0.5, "verdict": "unknown"}
    try:
        results_data = json.loads(results)
        web_results = results_data.get("data", {}).get("web", [])
        verdict = pipeline.skill_bank.rf_judge(query, web_results)
        return {
            "quality_pass": verdict.get("confidence", 0) >= 0.4,
            "rf_confidence": verdict.get("confidence", 0),
            "verdict": verdict.get("verdict", "unknown"),
        }
    except Exception as exc:
        logging.getLogger(__name__).debug("[SearchSkill] RF Judge 失败: %s", exc)
        return {"quality_pass": True, "rf_confidence": 0.5, "verdict": "parse_error"}
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if provider is None:
            ...
            # ── SearchSkill SRA Select 阶段 ───────────────────────────
            sra_ctx = _ss_sra_preprocess(query)
            logger.info(
                "Web search via %s: '%s' (limit: %d) | SRA难度: %s | 技能: %s",
                provider.name, query, limit,
                sra_ctx["difficulty"],
                [s["name"] for s in sra_ctx.get("selected_skills", [])],
            )
            response_data = provider.search(query, limit)
            # ── SearchSkill Select 阶段结束 ───────────────────────

        debug_call_data["results_count"] = len(response_data.get("data", {}).get("web", []))
        result_json = json.dumps(response_data, indent=2, ensure_ascii=False)

        # ── SearchSkill RF Judge 阶段 ─────────────────────────────
        rf = _ss_rf_judge(query, result_json, sra_ctx)
        if not rf.get("quality_pass", True):
            logger.warning("[SearchSkill] RF Judge 低质量告警: 置信度=%.2f verdict=%s",
                          rf.get("rf_confidence", 0), rf.get("verdict", "unknown"))
            response_data["_ss_quality"] = {
                "pass": False,
                "confidence": rf.get("rf_confidence", 0),
                "verdict": rf.get("verdict", "unknown"),
            }
            result_json = json.dumps(response_data, indent=2, ensure_ascii=False)
        # ── SearchSkill Judge 阶段结束 ───────────────────────────
