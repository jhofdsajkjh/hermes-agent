...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _on_message_recalled(self, event):
        ...
        # Extract session_id components. recalled_messages list contains
        # the message_ids, but we need the sender/chat to find the session.
        # However, GatewayRunner tracks agents by session_key.
        # Most Feishu sessions are keyed as "feishu:<open_id>"
        
        # Try to find a matching session via the singleton runner
        from gateway.run import _gateway_runner_ref
        runner_ref = _gateway_runner_ref
        runner = runner_ref() if callable(runner_ref) else None
        if not runner:
            return
        # Re-broadcast interrupt to all potentially matching sessions
        # since recalled_v1 event lacks the chat/user context in some SDK versions.
        # A more precise fix involves tracking message_id -> session_id mapping.
        # For now, we search for active agents.
        for session_key in list(runner._running_agents.keys()):
            if session_key.startswith("feishu:"):
                runner.request_interrupt(session_key)
